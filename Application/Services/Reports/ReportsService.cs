...
namespace ControleMercadoria.Application.Services.Reports
{
    public class ReportsService : IReportsService
    {
        ...
        public async Task<byte[]> GeneratePdfByFilter(
            ReportPeriodFilterDTO? dtoFilter, long userId)
        {
            ...
            var filtroTotalGasto = entryMovements.Where(x =>
                x.CreatedAt.ToString("dd/MM/yyyy") == inicio?.ToString("dd/MM/yyyy") ||
                x.CreatedAt.ToString("dd/MM/yyyy") == fim?.ToString("dd/MM/yyyy"));

            var filtroTotalVendido = exitMovements.Where(x =>
                x.CreatedAt.ToString("dd/MM/yyyy") == inicio?.ToString("dd/MM/yyyy") ||
                x.CreatedAt.ToString("dd/MM/yyyy") == fim?.ToString("dd/MM/yyyy"));

            var totalSpent = filtroTotalGasto.Sum(x => x.TotalValue);
            var totalSold = filtroTotalVendido.Sum(x => x.TotalValue);
            ...
            var summary = await GetSummaryReport(dtoFilter, summaryReports, userId);

            return await GeneratePdf(userId, dtoFilter, summary);
        }
